...
#define TH 20					// global threshold (intensity value) percentage 
#define BF_th 3					// candidate need larger than BF_th
...
//extern float cut_thre;
//extern bool thres_flag;
char header[54] = {
	...
	while (fin_in >> sFileName)
	{
		if (strcmp(sFileName.c_str(), "") != 0)
		{
			...
			for (int dotime = 0; dotime < 1 && !stop; dotime++)				
			{
				...
				foutattr <<" total " << iSoma_ALL_index << " candidates" << endl;
				...
				while (first_time < 2)
				{
					...
					for (int i = 0; i < iSoma_ALL_index; i++)
					{
						...
						if (judge > max_judge && x_train[0]>0.3 )  max_judge = judge;

						if (first_time == 1)
							foutattr << iSoma_ALL_attr[i].attr1 << "\t" << iSoma_ALL_attr[i].attr2 << "\t" << iSoma_ALL_attr[i].attr3 << "\t" << iSoma_ALL_attr[i].attr4 << "\t" << iSoma_ALL[i].x << "\t" << iSoma_ALL[i].y << "\t" << iSoma_ALL[i].z << "\t" << iSoma_ALL[i].value << "\t" << judge << "\t" << iSoma_ALL[i].bf_value << std::endl;
...
void do_attributes(int components, int raw_voxels)
{
	...
	for (int com = 0; com <components; com++)								//isolation
	{
		...
		for (int num = 0; num<icount_update; num++)
		{
			...
			//std::cout << " num " << num;
			if (x>0 && x<csVoxels.idim_x && y>0 && y<csVoxels.idim_y && z>0 && z<csVoxels.idim_z && csVoxels.Data[z][y][x].value > 0 && csVoxels.Data[z][y][x].bf_value > 3) 
			{
...
void do_candidate(int components)
{
	...
	for (int c = 0; c<components; c++)								//isolation
	{
		...
		//   fouta<<std::endl<<buffers<<std::endl;
		csVoxels.Get_Component(AM_all_data, c + 1, bound_count);
		//	if(c==0) csVoxels.Save_AM("testoutput.am");
		cout << "No. " << c + 1 << " component has voxels : " << csVoxels.iVoxels << std::endl;
		...
		while (bvalue>csVoxels.iBF_Max / 5 && bvalue > BF_th)
		{
			...
			if (find)
			{
				...
				if (union_index > union_thres)
				{
					...
					for (int i = 0; i<union_index; i++)
					{
						//if((union_find[i].bf_value == bvalue ||  union_find[i].bf_value == bvalue-1 )) // && abs(union_find[i].value-tmp_ity)<10)  // problem!!!?
						//if(union_find[i].value==tmp_ity)
				...
				//	iSoma_loc[icount++] = csVoxels.Data[union_find[0].z][union_find[0].y][union_find[0].x];
				}
